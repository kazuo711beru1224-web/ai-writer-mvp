...
def _build_writing_prompt(plan: str) -> str:
    ...
    if _is_money_contract_topic():
        p.append("【お金・契約テーマの追加ルール】")
        p.append("・手数料、返金、契約、解約、請求、カード、年会費などの条件は、断定しすぎないでください。")
        p.append("・『発生します』『請求されます』『可能です』『返金されます』『異議申し立てできます』のような言い切りは避けてください。")
        p.append("・根拠に明記されていない条件は、『発生する場合があります』『請求対象になる可能性があります』『確認が必要です』『問い合わせて確認するのが安全です』のように書いてください。")
        p.append("・カードの種類、契約時期、規約、キャンペーン条件、利用状況によって条件が異なる場合があることを書き添えてください。")
        p.append("・異議申し立て、解約、返金などの手続きができるかどうかは、根拠に明記されていない限り断定しないでください。")
        p.append("・読者がまず確認すべきものとして、利用明細、会員規約、公式サイト、カード会社の窓口を案内してください。")
        p.append("・『休眠状態』『顧客の責任』『異議申し立て』のような、根拠に明記されていない専門用語・制度用語・原因説明を勝手に作らないでください。根拠にない場合は『利用がない場合に手数料の対象となる場合があります』『請求理由は会員規約や利用明細で確認してください』のように、一般的で安全な表現にしてください。")
        p.append("")
